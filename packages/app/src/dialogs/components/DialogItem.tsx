...
class DialogItemImpl extends React.PureComponent<Props, {}> {
  render() {
    const { dialog, onClickDialog, applicationName, themeColor } = this.props;
    const ctaOnBottom = actionCTAOnBottom(this.props);
    ...
    const iconStyle: React.CSSProperties = {
      ...
      flexShrink: 0,
      marginTop: 2,
      backgroundImage: `url(${getApplicationIconURL(getDialogApplication(dialog))})`,
    ...
    return (
      <div style={containerStyle}>
        ...
        <div style={buttonWrapperStyle}>
          {
            getDialogActions(dialog).map((action, index) => {
